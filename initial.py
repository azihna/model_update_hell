...
df_merged = df_merged.drop(["supplier_id", "order_id", "product_id"], axis=1)

# %%
# how many orders per month in general?
df_merged = df_merged[
    df_merged.order_date.between(
        pd.to_datetime("2019-10-01"), pd.to_datetime("2021-10-01")
    )
]
...
df_init = df_merged[
    df_merged.order_date.between(
        pd.to_datetime("2019-10-01"), pd.to_datetime("2021-04-01")
    )
]
...
# %%
df_daily = df_merged[df_merged.order_date > pd.to_datetime("2021-04-01")]
